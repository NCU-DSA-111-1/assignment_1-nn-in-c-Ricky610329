...
int main()
{
	...
	FILE *fptr;
	fptr = fopen("./2bitslossdata.txt","w+");
  ...
  for(int i = 0; i < 25000; ++i) {
    ...
    sumofloss = 0;
    for(int j = 0; j < 4; ++j){
    	for (int i = 0; i < xor_ann->output_layer->num_outputs; i++) {
    		sumofloss += sigmoid(xor_ann->output_layer->outputs[i]) * (targets[i] - xor_ann->output_layer->outputs[i]);
  		}
  		sprintf(str,"%lf",sumofloss);
